    ...
    @foreach ($shelter_bill as $info)
        ...
        @if ($info->parts && count($info->parts) > 0 && $info->parts[0]->items)
            <table class="table table-bordered table-hover" id="boq-version-table">
                ...
                <tbody>
                    <tr class="text-bold">
                        <td colspan="{{$colspan}}" class="text-center no-border">Local Government Engineering Department (LGED)
                        </td>
                    </tr>
                    <tr class="text-bold">
                    ...
                    <tr class="text-bold">
                        <td colspan="{{$colspan}}" class="text-center no-border">Package: {{ $info->scheme->package->name }}
                    ...
                    <tr class="text-bold">
                        <td colspan="{{$colspan}}" class="text-center no-border">{{ $bill->name }}</td>
                    </tr>
                    <tr class="text-bold">
                    ...
                    @foreach ($info->parts as $part)
                            ...
                            @if (!$item->item->has_sub_items)
                                @php
                                    $boq_total += $item->boq_version_details
                                        ? $item->boq_version_details->quantity * $item->boq_version_details->rate
                                        : 0;
                                    $total += $item->bill_detail
                                        ? ($item->bill_detail->quantity - $item->bill_detail->held_up_quantity) *
                                            $item->boq_version_details->rate
                                        : 0;
                                    $this_bill_total += $item->bill_detail
                                        ? $item->bill_detail->this_bill_quantity * $item->boq_version_details->rate
                                        : 0;
                                    $previous_bill_total += $item->bill_detail
                                        ? $item->bill_detail->previous_quantity * $item->boq_version_details->rate
                                        : 0;
                                @endphp
                                <tr>
                                    ...
                                    <td class="text-right">
                                        {{ $item->bill_detail && $item->bill_detail->quantity - $item->bill_detail->held_up_quantity > 0 ? number_format($item->bill_detail->quantity - $item->bill_detail->held_up_quantity, 3) : '' }}
                                    </td>
                                    <td class="text-right">
                                        {{ $item->bill_detail && ($item->bill_detail->quantity - $item->bill_detail->held_up_quantity) * $item->boq_version_details->rate > 0 ? number_format(($item->bill_detail->quantity - $item->bill_detail->held_up_quantity) * $item->boq_version_details->rate, 2) : '' }}
                                    </td>
                                    @if($last_bill)
                                    <td class="text-right">
                                        {{ $item->bill_detail && $item->bill_detail->previous_quantity > 0 ? number_format($item->bill_detail->previous_quantity, 3) : '' }}
                                    </td>
                                    <td class="text-right">
                                        {{ $item->bill_detail && $item->bill_detail->previous_quantity * $item->boq_version_details->rate > 0 ? number_format($item->bill_detail->previous_quantity * $item->boq_version_details->rate, 2) : '' }}
                                    </td>
                                    @endif
                                    <td class="text-right">
                                        {{ $item->bill_detail && $item->bill_detail->this_bill_quantity > 0 ? number_format($item->bill_detail->this_bill_quantity, 3) : '' }}
                                    </td>
                                    <td class="text-right">
                                        {{ $item->bill_detail && $item->bill_detail->this_bill_quantity * $item->boq_version_details->rate > 0 ? number_format($item->bill_detail->this_bill_quantity * $item->boq_version_details->rate, 2) : '' }}
                                    </td>
                                    <td></td>
                                </tr>
                            @else
                            <tr>
                                <td colspan="{{$colspan}}" class="text-bold">({{ $item->item->code }})
                                    {{ $item->item->name }}</td>
                            </tr>
                                @foreach ($item->sub_items as $sub_item)
                                    @php
                                        ...
                                        $boq_total += $sub_item->boq_version_details
                                            ? $sub_item->boq_version_details->quantity * $sub_item->boq_version_details->rate
                                            : 0;
                                        $total += $sub_item->bill_detail
                                            ? ($sub_item->bill_detail->quantity - $sub_item->bill_detail->held_up_quantity) *
                                                $sub_item->boq_version_details->rate
                                            : 0;
                                        $this_bill_total += $sub_item->bill_detail
                                            ? $sub_item->bill_detail->this_bill_quantity * $sub_item->boq_version_details->rate
                                            : 0;
                                        $previous_bill_total += $sub_item->bill_detail
                                            ? $sub_item->bill_detail->previous_quantity * $sub_item->boq_version_details->rate
                                            : 0;
                                    @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">{{ $sub_item->sub_item->unit ? $sub_item->sub_item->unit->code : '' }}</td>
                                        <td class="text-right">
                                        ...
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && $sub_item->bill_detail->quantity - $sub_item->bill_detail->held_up_quantity > 0 ? number_format($sub_item->bill_detail->quantity - $sub_item->bill_detail->held_up_quantity, 3) : '' }}
                                        </td>
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && ($sub_item->bill_detail->quantity - $sub_item->bill_detail->held_up_quantity) * $sub_item->boq_version_details->rate > 0 ? number_format(($sub_item->bill_detail->quantity - $sub_item->bill_detail->held_up_quantity) * $sub_item->boq_version_details->rate, 2) : '' }}
                                        </td>
                                        @if($last_bill)
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && $sub_item->bill_detail->previous_quantity > 0 ? number_format($sub_item->bill_detail->previous_quantity, 3) : '' }}
                                        </td>
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && $sub_item->bill_detail->previous_quantity * $sub_item->boq_version_details->rate > 0 ? number_format($sub_item->bill_detail->previous_quantity * $sub_item->boq_version_details->rate, 2) : '' }}
                                        </td>
                                        @endif
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && $sub_item->bill_detail->this_bill_quantity > 0 ? number_format($sub_item->bill_detail->this_bill_quantity, 3) : '' }}
                                        </td>
                                        <td class="text-right">
                                            {{ $sub_item->bill_detail && $sub_item->bill_detail->this_bill_quantity * $sub_item->boq_version_details->rate > 0 ? number_format($sub_item->bill_detail->this_bill_quantity * $sub_item->boq_version_details->rate, 2) : '' }}
                                        </td>
                                        <td></td>
                                    </tr>
                                @endforeach
                        ...
                        @endphp
                        <tr>
                            <td colspan="5" class="text-right text-bold">Sub Total Bill - ({{ $part->part->code }})
                                {{ $part->part->name }}=</td>
                            <td class="text-right text-bold">{{ $boq_total > 0 ? number_format($boq_total, 2) : '' }}
                            </td>
                            <td></td>
                            ...
                            <td class="text-right text-bold">
                                {{ $previous_bill_total > 0 ? number_format($previous_bill_total, 2) : '' }}</td>
                                @endif
                            <td></td>
                            <td class="text-right text-bold">
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td class="text-right text-bold">{{ $part_total > 0 ? number_format($part_total, 2) : '' }}
                        </td>
                        @if($last_bill)
                        <td></td>
                        <td class="text-right text-bold">
                            {{ $part_previous_bill_total > 0 ? number_format($part_previous_bill_total, 2) : '' }}</td>
                        @endif
                        <td></td>
                        <td class="text-right text-bold">
                        ...
                        @foreach ($part->items as $item)
                            @if (!$item->item->has_sub_items)
                                @if ($item->measurements && count($item->measurements) > 0)
                                    ...
                                    <tr>
                                        <td colspan="{{ $colspan }}" class="text-bold no-border">Name of Item:
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right text-bold">
                                            {{ number_format($item->measurements->sum('quantity'), 3) }}</td>
                                        <td></td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right text-bold">
                                            {{ number_format($item->bill_detail->previous_quantity, 3) }}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        @if (in_array('piece', json_decode($item->item->unit->fields)))
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right text-bold">
                                            {{ number_format($item->bill_detail->held_up_quantity, 3) }}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        @if (in_array('piece', json_decode($item->item->unit->fields)))
                                        ...
                                        <td class="text-right text-bold">
                                            {{ number_format($item->bill_detail->this_bill_quantity, 3) }}</td>
                                        <td></td>
                                    </tr>
                                @endif
                            @endif
