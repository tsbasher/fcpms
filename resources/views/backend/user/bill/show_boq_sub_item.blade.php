                ...
                <div class="card">
                    <div class="card-header p-2">
                        <ul class="nav nav-pills">
                            ...
                            <li class="nav-item"><a class="nav-link active click_tab" href="{{ route('user.bills.details.boq_subitem', [$bill->id]) }}" data-toggle="tab">BOQ Sub Items</a></li>
                    ...
                    <div class="card-body" style="min-height: 80vh;">
                        <div class="tab-content">
                            <div class="active tab-pane" id="scheme">
                                ...
                                <div class="mb-2">
                                    <form role="form" method="POST" action="{{ route('user.bills.store_boq_subitems', $bill->id) }}" enctype="multipart/form-data">
                                        @csrf
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" name="boq_item_id" id="boq_item_id" placeholder="Select BOQ Items" required>
                                                    ...
                                                    @foreach ($bill_items as $bill_item)
                                                        <option value="{{ $bill_item->boq_item->id }}" @if (Request::get('boq_item_id') == $bill_item->boq_item->id) selected @endif>{{ $bill_item->boq_item->code }} - {{ $bill_item->boq_item->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                <label for="boq_subitems">BOQ Subitem</label>
                                                <select class="form-control select2" multiple name="boq_subitems[]" id="boq_subitems" placeholder="Select BOQ Subitems" required>
                                                    <option value="">Select BOQ Subitems</option>
                                                    @foreach ($boq_subitems as $boq_subitem)
                                                        <option value="{{ $boq_subitem->id }}" @if (in_array($boq_subitem->id, $bill->bill_subitems->pluck('boq_subitem_id')->toArray())) selected @endif>{{ $boq_subitem->code }} - {{ $boq_subitem->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover table-head-fixed" id="boq_part-table">
                                        ...
                                        <tbody>
                                            @foreach ($bill->bill_subitems as $item)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>
                                                    ...
                                                    <td>{{ $item->boq_subitem->code ?? 'N/A' }} -
                                                        {{ $item->boq_subitem->name ?? '' }}</td>
                                                    <td>
                                                        <a class="btn btn-sm btn-danger remove" data-url="{{ route('user.bills.remove_boq_subitem', ['id' => $item->id, 'bill_id' => $bill->id]) }}">X</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function generateurl(params) {
            url = "{{ route('user.bills.details.boq_subitem', $bill->id) }}";
