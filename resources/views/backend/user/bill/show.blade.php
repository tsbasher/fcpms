    ...
    <div class="col-md-3">
      ...
      <div class="card card-primary card-outline">
        ...
        <div class="card-body box-profile">
          ...
          <a href="{{ route('user.bills.edit', $bill->id) }}" class="btn btn-warning btn-block"><b>Edit Bill</b></a>
        </div>
        <!-- /.card-body -->
      </div>
      <!-- /.card -->
      ...
      <div class="card">
        <div class="card-header p-2">
          <ul class="nav nav-pills">
            <li class="nav-item"><a class="nav-link active" href="#scheme" data-toggle="tab">Scheme</a></li>
            <li class="nav-item"><a class="nav-link" href="#boq_parts" data-toggle="tab">BOQ Parts</a></li>
            <li class="nav-item"><a class="nav-link" href="#boq_items" data-toggle="tab">BOQ Items</a></li>
            <li class="nav-item"><a class="nav-link" href="#boq_sub_items" data-toggle="tab">BOQ Sub Items</a></li>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="scheme">
              <div class="mb-2 text-right">
                <a href="{{ route('user.bills.add_scheme', $bill->id) }}" id="add_scheme" class="btn btn-success"><i class="fa fa-plus"></i> Add/Remove Scheme</a>
              </div>
                ...
                <table class="table table-bordered table-hover table-head-fixed" id="packages-table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <a class="btn btn-sm btn-danger remove" data-url="{{ route('user.bills.remove_scheme', ['scheme_id' => $scheme->id, 'bill_id' => $bill->id]) }}">X</a>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
            ...
            <div class="tab-pane" id="boq_parts">
              <div class="mb-2 text-right">
                <a href="{{ route('user.bills.add_boq_parts', $bill->id) }}" id="add_boq_part" class="btn btn-success"><i class="fa fa-plus"></i> Add/Remove BOQ Part</a>
              </div>
              ...
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select class="form-control" id="scheme" name="scheme" disabled>
                      <option value="">Select Scheme</option>
                      @foreach ($schemes as $scheme)
                        <option value="{{ $scheme->id }}" {{ Request::get('scheme') == $scheme->id ? 'selected' : '' }}>{{ $scheme->code }} - {{ $scheme->name }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-hover table-head-fixed" id="boq_part-table">
                  ...
                  <tbody>
                    @foreach ($bill->boq_parts as $part)
                    <tr>
                      <td>{{ $loop->index + 1 }}</td>
                      <td>{{ $part->name }}</td>
                      <td>{{ $part->code }}</td>
                      <td>{{ $part->package->name ?? 'N/A' }}</td>
                      <td>{{ $scheme->district->name ?? 'N/A' }}</td>
                      <td>{{ $scheme->upazila->name ?? 'N/A' }}</td>
                      <td>{{ $scheme->union->name ?? 'N/A' }}</td>
                      <td>
...
<script type="text/javascript">
  ...
  $("#add_scheme").click(function(e) {
    ...
    $("#modal-add").modal({show: true,backdrop: 'static', keyboard: false});
  });
